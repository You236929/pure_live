...
mixin StripChatSiteMixin on LiveSite {
  @override
  String getJumpToNativeUrl(LiveRoom liveRoom) {
    try {
      var appUrl = "stripchat://player/live?broad_no=${liveRoom.userId}&user_id=${liveRoom.roomId}&channel=";
      return appUrl;
    } catch (e) {
      return "";
    }
  ...
  String getJumpToWebUrl(LiveRoom liveRoom) {
    try {
      var webUrl = "https://zh.stripchat.com/${liveRoom.roomId}";
      return webUrl;
    } catch (e) {
      return "";
    }
  ...
  Future<SiteParseBean> parse(String url) async {
    ...
    List<RegExp> regExpBeanList = [
      RegExp(r"stripchat\.com/([^/]+)"),
      RegExp(r"live\.uaa\.com/([^/]+)"),
    ];
