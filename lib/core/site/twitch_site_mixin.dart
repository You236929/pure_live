...
mixin TwitchSiteMixin on LiveSite {
  @override
  String getJumpToNativeUrl(LiveRoom liveRoom) {
    try {
      var appUrl = "twitch://player/live?broad_no=${liveRoom.userId}&user_id=${liveRoom.roomId}&channel=";
      return appUrl;
    } catch (e) {
      return "";
    }
  ...
  String getJumpToWebUrl(LiveRoom liveRoom) {
    try {
      var webUrl = "https://www.twitch.tv/${liveRoom.roomId}";
      return webUrl;
    } catch (e) {
      return "";
    }
  ...
  List<OtherJumpItem> jumpItems(LiveRoom liveRoom) {
    ...
    list.add(OtherJumpItem(
      ...
      onTap: () async {
        try {
          await launchUrlString("https://www.twitch.tv/${liveRoom.roomId}/vods", mode: LaunchMode.externalApplication);
        } catch (e) {
          CoreLog.error(e);
        }
      },
